...
const createClient = async (req, res) => {
    ...
    try {
        ...
        const newClient = new client_model_1.default({ clientId, status: 'INITIALIZED' });
...
const connectClient = async (req, res) => {
    ...
    try {
        ...
        const clientData = await client_model_1.default.findOne({ clientId });
        if (!clientData) {
...
const deleteClient = async (req, res) => {
    ...
    try {
        ...
        const client = await client_model_1.default.findOne({ clientId });
        if (!client) {
            return res.status(http_status_codes_1.StatusCodes.NOT_FOUND).json({ success: false, error: 'Client not found.' });
        }
        await client_service_1.default.disconnectClient(clientId);
...
const getClientById = async (req, res) => {
    ...
    try {
        const { clientId } = req.params;
        const client = await client_model_1.default.findOne({ clientId }).select('-session');
        if (!client) {
            return res.status(http_status_codes_1.StatusCodes.NOT_FOUND).json({ success: false, error: 'Client not found.' });
        }
