...
exports.startOnboarding = exports.sendImageByName = exports.sendImageFromDb = exports.sendImageMessage = exports.deleteUser = exports.getAllUsers = exports.activateUser = exports.addUser = void 0;
...
const sendImageByName = async (req, res) => {
    try {
        const { clientId, wa_num, imageName, caption } = req.body;
        if (!clientId || !wa_num || !imageName) {
            return res.status(http_status_codes_1.StatusCodes.BAD_REQUEST).json({ success: false, error: 'clientId, wa_num, and imageName are required.' });
        }
        const client = client_service_1.default.getClient(clientId);
        if (!client) {
            return res.status(http_status_codes_1.StatusCodes.SERVICE_UNAVAILABLE).json({ success: false, error: 'WhatsApp client is not connected.' });
        }
        const imageDoc = await image_model_1.default.findOne({ name: imageName });
        if (!imageDoc) {
            return res.status(http_status_codes_1.StatusCodes.NOT_FOUND).json({ success: false, error: `Image with name '${imageName}' not found.` });
        }
        const jid = wa_num.toString().includes('@') ? wa_num.toString() : `${wa_num}@s.whatsapp.net`;
        await client.sendMessage(jid, {
            image: imageDoc.data,
        ...
        return res.status(http_status_codes_1.StatusCodes.INTERNAL_SERVER_ERROR).json({ success: false, error: 'Failed to send image by name.' });
    }
};
exports.sendImageByName = sendImageByName;
/**
...
const startOnboarding = async (req, res) => {
    ...
    try {
        ...
        const result = await user_service_1.default.startOnboarding({
            user_name,
            wa_num: waNumber
        });
        if (!result.success) {
