...
class ClientService {
    ...
    async processWebhookAsync(clientId, messages) {
        // Process webhook in background without blocking
        setImmediate(async () => {
            try {
                const clientData = await client_model_1.default.findOne({ clientId }).lean();
                if (!clientData)
                    return;
                for (const message of messages) {
                    ...
                    if (clientData.client_type === 'translate') {
                        ...
                        if (phoneNumber && !isNaN(phoneNumber)) {
                            user = await user_model_1.default.findOne({ wa_num: phoneNumber }).lean();
