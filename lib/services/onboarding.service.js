    ...
    async handleValidInput(user, messageText, clientId, recipient, stepName) {
        try {
            ...
            if (finalFieldToUpdate === 'country') {
                ...
                const canonicalCountryName = await country_service_1.default.getCanonicalCountryName(messageText);
                // Get spending power from country service
                let spendingPower = await country_service_1.default.getSpendingPower(messageText);
    ...
    async moveToNextStep(user, clientId, recipient, currentStep) {
        try {
            ...
            if (nextStep) {
                ...
                const updatedUser = await user_model_1.default.findById(user._id).lean();
                if (!updatedUser) {
                ...
                try {
                    if (updatedUser.segmentation) {
                        ...
                        userContext = await context_service_1.default.generateFullContext(updatedUser.segmentation);
                ...
                const result = await user_model_1.default.findByIdAndUpdate(updatedUser._id, {
                    status: 'ACTIVE',
                    current_step: null,
                    context: userContext
                }, { new: true });
                ...
                const completionMessage = await predefined_service_1.default.getMessage('onboarding_complete', 'welcome');
                if (completionMessage) {
                    ...
                    await messaging_service_1.default.sendRawTextMessage(clientId, recipient, message);
                }
                else {
