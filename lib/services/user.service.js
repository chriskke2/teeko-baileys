...
class UserService {
    ...
    async processIncomingMessage(message, clientId) {
        try {
            ...
            if (!user) {
                ...
                if (!config_1.default.subscribe_required) {
            ...
            if (!user.first_name || (firstName && user.first_name !== firstName)) {
                ...
                await user_model_1.default.updateOne({ _id: user._id }, { first_name: firstName });
            }
    ...
    async handleActiveUserMessage(user, messageText, clientId, recipient, senderName) {
        try {
            ...
            const fallbackFunction = async () => {
                ...
                await messaging_service_1.default.sendRawTextMessage(clientId, recipient, "We're processing your request. Please wait a moment...");
            };
            ...
            const responseHandler = async (response) => {
                if (response && response.message) {
                    ...
                    try {
                        ...
                        try {
                            await messaging_service_1.default.sendRawTextMessage(clientId, recipient, "We received your message but encountered an issue sending the full response. Please try again.");
                        }
                        catch (retryError) {
                    ...
                    try {
                        await messaging_service_1.default.sendRawTextMessage(clientId, recipient, "Sorry, we couldn't process your request at this time. Please try again later.");
                    }
                    catch (error) {
                    ...
                    try {
                        await messaging_service_1.default.sendRawTextMessage(clientId, recipient, "Thank you for your message. We're processing your request.");
                    }
                    catch (error) {
            ...
            const webhookSent = await webhook_service_1.default.sendMessageWebhook(webhookPayload, fallbackFunction, responseHandler);
            if (!webhookSent) {
                ...
                await messaging_service_1.default.sendRawTextMessage(clientId, recipient, "We're having trouble connecting to our services. Please try again later.");
            }
        }
        catch (error) {
     ...
     */
    async createUserForDirectOnboarding(waNumber, senderName) {
        try {
            // Extract first name from sender name
            const firstName = senderName ? senderName.split(' ')[0] : '';
            // Check if onboarding steps exist
            const onboardingSteps = await predefined_service_1.default.getAllByType('onboarding');
            ...
            if (hasOnboardingSteps) {
                const sortedSteps = onboardingSteps.sort((a, b) => {
            ...
            const newUser = new user_model_1.default({
                ...
                status: hasOnboardingSteps ? activation_service_2.UserState.ONBOARDING : activation_service_2.UserState.ACTIVE,
     ...
     * @param recipient Recipient JID
     */
    async handleNewUserWelcome(user, clientId, recipient) {
        try {
            const firstName = user.first_name || 'there';
            if (user.status === activation_service_2.UserState.ONBOARDING && user.current_step) {
                // Send welcome message and start onboarding
                await messaging_service_1.default.sendRawTextMessage(clientId, recipient, `Hi ${firstName}. I am Teeko! Let's get you set up with a few quick questions. It only takes 1 minute!`);
                // Send the first onboarding question
                const message = await predefined_service_1.default.getMessage('onboarding', user.current_step);
                const hasOptions = message && message.options && message.options.length > 0;
                if (hasOptions) {
                    await messaging_service_1.default.sendOptionsMessage(user.current_step, 'onboarding', clientId, recipient);
                }
                else {
                    await messaging_service_1.default.sendMessage(`onboarding/${user.current_step}`, clientId, recipient);
                }
            }
            else {
                // No onboarding needed, user is active
                await messaging_service_1.default.sendRawTextMessage(clientId, recipient, `Hi ${firstName}! Welcome to our chatbot. How can I help you today?`);
            }
        }
        catch (error) {
            console.error('Error handling new user welcome:', error);
            // Send a simple welcome message as fallback
            await messaging_service_1.default.sendRawTextMessage(clientId, recipient, "Welcome! How can I help you today?");
        }
    }
