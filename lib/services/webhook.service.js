...
class WebhookService {
    ...
    async sendMessageWebhookUnified(message, clientId, clientType, user) {
        try {
            ...
            return await this.sendWebhookWithResponse(webhookUrl, webhookPayload, clientId, message.key?.remoteJid || '', phoneNumber, clientType);
        }
        catch (error) {
     ...
     * @returns Promise<boolean> Success status
     */
    async sendWebhookWithResponse(webhookUrl, payload, clientId, remoteJid, phoneNumber, clientType) {
        try {
     ...
     */
    async processTaxiData(taxiData) {
        try {
